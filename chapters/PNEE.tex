...
\unsure{Algorithm 1 is not done (formating/naming). Should I include this algorithm or leave it out entirely? There is also no textreference for now as I tend to leave it out.}

\begin{algorithm}
\caption{Get interpolated Distribution}\label{alg:trilinear}
\begin{algorithmic}[1]
\Procedure{getInterpolatedDistribution}{$pos$}
\State $distributions$ $\gets$ getDistribution($pos$)
\State $voxelPos$ $\gets$ $pos$
\State $influences$ $\gets$ [1.0]
\For {$i \gets 1; i < 3; ++i$}
    \If {$offsetInVoxel == 0.f$} continue
    \EndIf
    \State $offsetInVoxel \gets (fmod(offset[i] / (1.0f / nVoxels[i]), 1.0f)) - 0.5f$
    \State $size \gets voxelIds.size()$
    \For {$n \gets 0; n < size; ++n$}
        \State newId[i] += offsetInVoxel > 0? 1 : -1
        
        \If {$newId[i] >= 0 and newId[i] < nVoxels[i]$} 
            \State $distributions.push_back(getDistribution(newPackedPos, newHash, nProbes))$
            \State $voxelIds.pushback(newId)$
            \State $influence.pushback(influence[n] * abs(offsetInVoxel))$
            \State $influence[n] *= (1 - abs(offsetInVoxel))$
        \EndIf
